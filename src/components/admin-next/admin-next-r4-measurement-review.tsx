...
export function AdminNextR4MeasurementReview({ address, locale, caseReference, customer, measurement, owner, returnTo, source = "fixture" }: { address: string; locale: PanelLocale; caseReference: string; customer: string; measurement: MeasurementReview; owner: string; returnTo?: string; source?: MeasurementSource }) {
  const t = copy[locale];
  const stateText = stateCopy[locale];
  const returnHref = returnTo ?? `/admin-next-preview/cases/${encodeURIComponent(caseReference)}`;
      ...
      <aside aria-labelledby="r4-drawer-title" aria-modal="true" className="fixed bottom-[4.5rem] right-0 top-16 z-50 flex w-full flex-col overflow-hidden border-l border-[var(--an-border-strong)] bg-[var(--an-surface)] shadow-[-24px_0_70px_rgba(0,0,0,.48)] lg:bottom-0 lg:w-[760px] xl:w-[820px]" role="dialog">
        <header className="shrink-0 border-b border-[var(--an-border)] bg-[var(--an-sidebar)] px-4 py-4 sm:px-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <Link aria-label={t.close} className="grid size-11 shrink-0 place-items-center rounded-xl border border-[var(--an-border)] bg-[var(--an-elevated)] text-[var(--an-muted)] hover:text-[var(--an-amber)]" href={returnHref}><X aria-hidden="true" className="size-5" /></Link>
          </div>
