...
const copy = {
  nb: {
    ...
    received: "Mottatt", reason: "Kundens valgte årsak", comment: "Kundens kommentar", consent: "Tillater én løsningsorientert oppfølging", noConsent: "Ingen samtykke til salgsoppfølging", deadline: "Nominell 14-dagersfrist", inside: "Mottatt innen nominell frist", outside: "Mottatt etter nominell frist – må vurderes, ikke avvis automatisk", unknown: "Kan ikke beregnes automatisk", work: "Arbeidsstatus ved mottak", suggestion: "Systemets forslag", basis: "Administrators skriftlige beslutningsgrunnlag", followDate: "Planlagt oppfølging", close: "Bekreft avslutning og send kunden", continue: "Avklar og fortsett", alternative: "Forbered alternativt tilbud", schedule: "Planlegg oppfølging", doNotContact: "Avslutt – ikke kontakt", processing: "Behandler …", success: "Avgjørelsen er lagret.", closeSuccess: "Avtalen er avsluttet, og kunden har fått eller vil straks få skriftlig bekreftelse.", failed: "Avgjørelsen kunne ikke lagres.", confirm: "Bekreft at opplysningene, arbeidet og eventuell betaling er kontrollert før beslutningen lagres.", confirmClose: "Avtalen avsluttes, eventuell arbeidsstart stoppes, og kunden får automatisk skriftlig bekreftelse på norsk. Bekreft at arbeid og mulig betaling er kontrollert.", green: "Grønn", yellow: "Gul", red: "Rød",
  },
  lt: {
    ...
    received: "Gauta", reason: "Kliento pasirinkta priežastis", comment: "Kliento komentaras", consent: "Leidžia vieną kontaktą sprendimui pasiūlyti", noConsent: "Nėra sutikimo pardavimo kontaktui", deadline: "Nominalus 14 dienų terminas", inside: "Gauta per nominalų terminą", outside: "Gauta po nominalaus termino – būtina įvertinti, neatmesti automatiškai", unknown: "Automatiškai apskaičiuoti negalima", work: "Darbo būsena gavimo metu", suggestion: "Sistemos pasiūlymas", basis: "Administratoriaus rašytinis sprendimo pagrindas", followDate: "Planuojamo kontakto data", close: "Patvirtinti atsisakymą ir išsiųsti klientui", continue: "Išspręsta – tęsti", alternative: "Paruošti alternatyvų pasiūlymą", schedule: "Suplanuoti kontaktą", doNotContact: "Užbaigti – nebekontaktuoti", processing: "Vykdoma …", success: "Sprendimas išsaugotas.", closeSuccess: "Sutartis užbaigta, o klientui išsiųstas arba nedelsiant bus išsiųstas rašytinis patvirtinimas.", failed: "Sprendimo išsaugoti nepavyko.", confirm: "Patvirtinkite, kad prieš išsaugant sprendimą patikrinote sutartį, darbus ir galimą mokėjimą.", confirmClose: "Sutartis bus užbaigta, suplanuoti darbai sustabdyti, o klientui automatiškai išsiųstas rašytinis patvirtinimas norvegų kalba. Patvirtinkite, kad patikrinote darbų ir galimo mokėjimo būseną.", green: "Žalia", yellow: "Geltona", red: "Raudona",
  },
  en: {
    title: "Withdrawal or change notice requires review", withdrawal: "Exercise of withdrawal right", change: "Change or cancellation",
    received: "Received", reason: "Customer-selected reason", comment: "Customer comment", consent: "Allows one solution-oriented follow-up", noConsent: "No consent for sales follow-up", deadline: "Nominal 14-day deadline", inside: "Received within the nominal period", outside: "Received after the nominal period – review, do not reject automatically", unknown: "Cannot be calculated automatically", work: "Work status at receipt", suggestion: "System suggestion", basis: "Administrator's written decision basis", followDate: "Planned follow-up", close: "Confirm withdrawal and notify customer", continue: "Resolve and continue", alternative: "Prepare alternative quote", schedule: "Schedule follow-up", doNotContact: "Close – do not contact", processing: "Processing …", success: "Decision saved.", closeSuccess: "The agreement is closed and the customer has received or will immediately receive written confirmation.", failed: "The decision could not be saved.", confirm: "Confirm that the contract, work and any payment have been checked before saving the decision.", confirmClose: "The agreement will be closed, planned work stopped, and the customer automatically sent written confirmation in Norwegian. Confirm that work and any payment have been checked.", green: "Green", yellow: "Yellow", red: "Red",
  },
} as const;
...
export function ContractRequestReviewPanel({ locale, request }: { locale: PanelLocale; request: CaseContractRequest }) {
  ...
  async function decide(decision: "close" | "continue" | "alternative" | "schedule_follow_up" | "do_not_contact") {
    ...
    if (!window.confirm(decision === "close" ? labels.confirmClose : labels.confirm)) return;
    ...
    try {
      ...
      setNotice(decision === "close" ? labels.closeSuccess : labels.success); router.refresh();
