...
import { blogGenerationError } from "@/lib/admin-v2/blog-generation-error";
...
export function BlogGenerateButton({ locale }: { locale: PanelLocale }) {
  ...
  async function generate(){setBusy(true);setNotice("");try{const response=await fetch("/api/admin/blog/generate",{method:"POST"});const result=await response.json() as {error?:string;postId?:number;code?:string;issues?:Array<{code:string;message:string}>};if(!response.ok)throw new Error(blogGenerationError(result,locale));if(result.postId)router.push(`/admin-v2/blog/${result.postId}`);else{setNotice(copy.actionDone);router.refresh();}}catch(error){setNotice(error instanceof Error?error.message:"Failed");}finally{setBusy(false);}}
