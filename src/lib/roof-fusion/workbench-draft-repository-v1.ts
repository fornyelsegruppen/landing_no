...
import type { RfDraftRecoveryBinding } from "@/lib/admin-next/rf-draft-recovery-contract";
import {
  parseWorkbenchDraftRecoveryRecordV1,
  serializeWorkbenchDraftRecoveryRecordV1,
  type RoofFusionWorkbenchDraftRecoveryRecordV1,
} from "./workbench-draft-recovery-v1";

export type WorkbenchDraftExpectedLatestV1 =
  RoofFusionWorkbenchDraftReferenceV1 | null;
...
export interface RoofFusionWorkbenchDraftRepositoryV1 {
  ...
  readDraftRecoveryRecord(
    caseId: string,
    draftId: string,
  ): Promise<RoofFusionWorkbenchDraftRecoveryRecordV1 | null>;
  readLatestDraftRecoveryRecord(
    caseId: string,
  ): Promise<RoofFusionWorkbenchDraftRecoveryRecordV1 | null>;
  readByIdempotency(
    caseId: string,
...
export class InMemoryRoofFusionWorkbenchDraftRepositoryV1 implements RoofFusionWorkbenchDraftRepositoryV1 {
  ...
  async readDraft(caseId: string, draftId: string) {
    return (await this.readDraftRecoveryRecord(caseId, draftId))?.draft ?? null;
  }

  async readLatestDraft(caseId: string) {
    return (await this.readLatestDraftRecoveryRecord(caseId))?.draft ?? null;
  }

  async readDraftRecoveryRecord(caseId: string, draftId: string) {
    return (
      (this.drafts.get(caseId) ?? []).find(
        (record) => record.draft.draftId === draftId,
      ) ?? null
    );
  }

  async readLatestDraftRecoveryRecord(caseId: string) {
  ...
  async readByIdempotency(caseId: string, idempotencyKey: string) {
    return (
      (this.drafts.get(caseId) ?? []).find(
        (record) => record.draft.idempotencyKey === idempotencyKey,
      )?.draft ?? null
    );
  }
  ...
  async appendAtomically({
    ...
    const replay = await this.readByIdempotency(
      parsed.caseId,
    ...
    if (ids.some((item) => item.draft.draftId === parsed.draftId)) {
      throw new RoofFusionWorkbenchDraftRepositoryError(
        "DRAFT_ID_CONFLICT",
        "Workbench draft ID already exists",
      );
    }
    const record = recoveryBinding
      ? parseWorkbenchDraftRecoveryRecordV1(
          serializeWorkbenchDraftRecoveryRecordV1({
            draft: parsed,
            recoveryBinding,
      ...
      : { draft: structuredClone(parsed), recoveryBinding: null };
    this.drafts.set(parsed.caseId, [...ids, record]);
    return "applied";
  }
}
...
function parseStoredDraftRecord(
  row: RoofFusionWorkbenchDraft,
): RoofFusionWorkbenchDraftRecoveryRecordV1 {
  let record: RoofFusionWorkbenchDraftRecoveryRecordV1;
  try {
    record = parseWorkbenchDraftRecoveryRecordV1(row.draft);
  } catch (error) {
    throw new RoofFusionWorkbenchDraftRepositoryError(
      "REPOSITORY_INTEGRITY",
...
export class PayloadRoofFusionWorkbenchDraftRepositoryV1 implements RoofFusionWorkbenchDraftRepositoryV1 {
  ...
  private async findRecord(
    where: Where,
    options: { sort?: string; req?: TransactionRequest } = {},
  ) {
    ...
    return row ? parseStoredDraftRecord(row) : null;
  }

  private readDraftRecordWithRequest(
    caseId: string,
    draftId: string,
    ...
    return this.findRecord(
      {
        and: [{ caseId: { equals: caseId } }, { draftId: { equals: draftId } }],
      },
      req ? { req } : {},
    );
  }
  ...
  readDraft(caseId: string, draftId: string) {
    return this.readDraftRecordWithRequest(caseId, draftId).then(
      (record) => record?.draft ?? null,
    );
  }

  readDraftRecoveryRecord(caseId: string, draftId: string) {
    return this.readDraftRecordWithRequest(caseId, draftId);
  }

  private readLatestRecordWithRequest(
    caseId: string,
    req?: TransactionRequest,
  ) {
    return this.findRecord(
      { caseId: { equals: caseId } },
      { sort: "-revision", ...(req ? { req } : {}) },
    );
  }

  readLatestDraft(caseId: string) {
    return this.readLatestRecordWithRequest(caseId).then(
      (record) => record?.draft ?? null,
    );
  }

  readLatestDraftRecoveryRecord(caseId: string) {
    return this.readLatestRecordWithRequest(caseId);
  }

  private readByIdempotencyWithRequest(
    caseId: string,
    idempotencyKey: string,
    req?: TransactionRequest,
  ) {
  ...
  async appendAtomically({
    ...
    if (!started)
      throw new RoofFusionWorkbenchDraftRepositoryError(
        "REPOSITORY_INTEGRITY",
        "Workbench draft append requires an independently owned transaction",
      );
    let committed = false;
    try {
      const latestRecord = await this.readLatestRecordWithRequest(
        parsed.caseId,
        request,
      );
      const latest = latestRecord?.draft ?? null;
      const replay = await this.readByIdempotencyWithRequest(
        parsed.caseId,
        parsed.idempotencyKey,
        request,
      );
      ...
      assertLineage(latest, parsed);
      if (
        await this.readDraftRecordWithRequest(
          parsed.caseId,
          parsed.draftId,
          request,
        )
      ...
      await this.payload.create({
        ...
        data: {
          ...
          ...(parsed.supersedesDraftId
            ? { supersedesDraftId: parsed.supersedesDraftId }
            : {}),
          draftHash: parsed.draftHash,
          idempotencyKey: parsed.idempotencyKey,
          state: parsed.state,
          sourceContentHash: parsed.source.sourceContentHash,
          draft: recoveryBinding
            ? serializeWorkbenchDraftRecoveryRecordV1({
                draft: parsed,
                recoveryBinding,
              })
