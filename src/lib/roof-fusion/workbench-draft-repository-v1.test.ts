...
function geometry(): AssistedManualRoofGeometryV1 {
  return {
    ...
    roofMasses: [{ massId: "mass-1", outlineId: "outline-1", approvedByActorId: "7", approvedAt: "2026-09-03T08:00:00.000Z", vertexIds: ["v1", "v2", "v3", "v4"] }],
    skeletonEdges: [{ edgeId: "ridge-1", roofMassId: "mass-1", fromVertexId: "ridge1", toVertexId: "ridge2", type: "ridge", provenance: "manual" }],
    openings: [],
    obstacles: [],
  };
}
  ...
  return buildRoofFusionWorkbenchDraftV1({
    ...
    source: {
      ...
      georeference: {
        ...
        bounds: { minEastingM: 499990, minNorthingM: 6639990, maxEastingM: 500020, maxNorthingM: 6640020 },
        imageWidth: 1920,
        imageHeight: 1080,
      },
    },
...
describe("Roof Fusion workbench draft repository v1", () => {
  it("appends consecutive revisions and replays the same idempotent command", async () => {
    ...
    const first = draft();
    expect(await repository.appendAtomically({ expectedLatest: null, draft: first })).toBe("applied");
    expect(await repository.appendAtomically({ expectedLatest: null, draft: first })).toBe("replayed");
    const second = draft(2);
    expect(await repository.appendAtomically({ expectedLatest: { draftId: first.draftId, revision: first.revision, draftHash: first.draftHash, state: first.state }, draft: second })).toBe("applied");
  ...
  it("rejects a stale revision expected latest", async () => {
    ...
    await expect(repository.appendAtomically({ expectedLatest: { draftId: first.draftId, revision: 99, draftHash: first.draftHash, state: first.state }, draft: draft(2) })).rejects.toMatchObject({ code: "EXPECTED_REVISION_MISMATCH" });
  });
  ...
  it("rejects a stale hash expected latest", async () => {
    ...
    const first = draft();
    await repository.appendAtomically({ expectedLatest: null, draft: first });
    await expect(repository.appendAtomically({ expectedLatest: { draftId: first.draftId, revision: first.revision, draftHash: "b".repeat(64), state: first.state }, draft: draft(2) })).rejects.toMatchObject({ code: "STALE_DRAFT_HASH" });
  });
  ...
  it("rejects a broken append-only revision lineage", async () => {
    ...
    const first = draft();
    await repository.appendAtomically({ expectedLatest: null, draft: first });
    await expect(repository.appendAtomically({
      expectedLatest: { draftId: first.draftId, revision: first.revision, draftHash: first.draftHash, state: first.state },
      draft: draft(2, { supersedesDraftId: "different-draft" }),
    })).rejects.toMatchObject({ code: "REPOSITORY_INTEGRITY" });
  });

  it("rejects idempotency reuse with different content and invalid source/georef", async () => {
    const repository = new InMemoryRoofFusionWorkbenchDraftRepositoryV1();
    const first = draft();
    await repository.appendAtomically({ expectedLatest: null, draft: first });
    const different = draft(2, { idempotencyKey: first.idempotencyKey });
    await expect(repository.appendAtomically({ expectedLatest: null, draft: different })).rejects.toMatchObject({ code: "IDEMPOTENCY_CONFLICT" });
    expect(() => draft(1, { source: { ...first.source, sourceContentHash: "b".repeat(64) } })).toThrow();
    expect(() => draft(1, { source: { ...first.source, georeference: { ...first.source.georeference, extentTrust: "planned-url-bounds" } } })).toThrow();
  });

  it("fails closed for invalid topology before repository append", () => {
    expect(() => draft(1, { geometry: { ...geometry(), roofMasses: [{ ...geometry().roofMasses[0], vertexIds: ["v1", "v1", "v2"] }] } })).toThrow();
