...
import { processOperationalJobs } from "./operational-job-processor";
...
function repository() {
  ...
  const payload = {
    async find({ collection, where }: { collection: string; where?: unknown }) {
      if (collection === "operational-jobs" && JSON.stringify(where).includes('"running"')) return { docs: [] };
...
describe("operational job processor", () => {
  ...
  afterEach(() => {
    if (originalEnvironment.VERCEL_ENV === undefined) delete process.env.VERCEL_ENV;
    else process.env.VERCEL_ENV = originalEnvironment.VERCEL_ENV;
    if (originalEnvironment.ALLOW_PREVIEW_EMAIL_LOG === undefined) delete process.env.ALLOW_PREVIEW_EMAIL_LOG;
    else process.env.ALLOW_PREVIEW_EMAIL_LOG = originalEnvironment.ALLOW_PREVIEW_EMAIL_LOG;
    if (originalEnvironment.RESEND_API_KEY === undefined) delete process.env.RESEND_API_KEY;
  ...
  it("delivers a referenced message immediately and completes its durable job", async () => {
    ...
    expect(result, JSON.stringify({ job: state.job, message: state.message, lead: state.lead })).toMatchObject({ completed: [3], attention: [], retried: [] });
    ...
    expect(JSON.stringify(state.job.payload)).not.toContain("kunde@example.test");
  });
