...
export const customerContractRequestSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (
      ...
      (!value.reasonText || value.reasonText.length < 3)
    ) {
      context.addIssue({
        code: "custom",
    ...
    if (value.preferredFollowUp === "custom" && !value.preferredFollowUpAt) {
      context.addIssue({
        ...
        path: ["preferredFollowUpAt"],
      });
    }
    if (
      !value.followUpConsent &&
      value.preferredFollowUp &&
      value.preferredFollowUp !== "never"
    ) {
      context.addIssue({
        code: "custom",
        message: "Follow-up timing requires consent",
        path: ["preferredFollowUp"],
      });
    }
  });

export type CustomerContractRequestInput = z.infer<
  typeof customerContractRequestSchema
>;

export const reasonLabelsNo: Record<
  (typeof contractRequestReasonCodes)[number],
  string
> = {
...
export function nominalWithdrawalAssessment(
  signedAt: string | null | undefined,
  now: Date,
) {
  if (!signedAt) return { deadline: undefined, withinPeriod: undefined };
  const deadline = new Date(
    new Date(signedAt).getTime() + 14 * 24 * 60 * 60_000,
  );
  return {
    deadline: deadline.toISOString(),
    withinPeriod: now.getTime() <= deadline.getTime(),
  };
}
...
function acknowledgementCopy(input: {
  customer: string;
  contractReference: string;
  kind: CustomerContractRequestInput["action"];
  receivedAt: string;
}) {
  const received = new Intl.DateTimeFormat("nb-NO", {
    dateStyle: "long",
    timeStyle: "short",
...
export async function recordCustomerContractRequest(
  payload: Payload,
  input: {
  ...
  if (existing.docs[0])
    return { duplicate: true as const, request: existing.docs[0] };
  ...
  const kindLabel =
    input.request.action === "withdrawal"
      ? "Bruk av angrerett"
  ...
  const bodyText = [
    ...
    `Årsak: ${reasonLabel}`,
    input.request.reasonText
      ? `Kundens kommentar: ${input.request.reasonText}`
      : null,
    `Samtykke til én oppfølging: ${input.request.followUpConsent ? "Ja" : "Nei"}`,
    input.request.preferredFollowUp
      ? `Ønsket oppfølgingstid: ${input.request.preferredFollowUp}`
      : null,
  ]
    .filter(Boolean)
    .join("\n");
  const sourceMessage = await payload.create({
    collection: "messages",
    overrideAccess: true,
    data: {
      lead: input.leadId,
  ...
  if (
    workOrder &&
    !["completed", "documented", "cancelled"].includes(workOrder.status)
  ) {
    await payload.update({
      ...
      overrideAccess: true,
      data: {
        statusBeforeCustomerCancellation:
  ...
  const customFollowUpAt = input.request.followUpConsent
    ? suggestedFollowUpAt(
        input.request.preferredFollowUp,
        input.request.preferredFollowUpAt,
        now,
      )
    : undefined;
  const earlyStartRequested =
    signatureValue(input.signatureEvidence, "earlyStartRequested") === true;
  const quotePricing =
    input.quoteSnapshot &&
    typeof input.quoteSnapshot === "object" &&
    "pricing" in input.quoteSnapshot
      ? (input.quoteSnapshot as { pricing?: Record<string, unknown> }).pricing
      : undefined;
  const depositBasisPoints =
    typeof quotePricing?.depositBasisPoints === "number"
      ? quotePricing.depositBasisPoints
      : 0;
  ...
  const requestRecord = await payload.create({
    ...
    data: {
      ...
      followUpConsent: input.request.followUpConsent,
      preferredFollowUp: input.request.followUpConsent
        ? input.request.preferredFollowUp || null
        : input.request.preferredFollowUp === "never"
          ? "never"
          : null,
      ...
      depositStatusAtReceipt:
        depositBasisPoints > 0 ? "required_unverified" : "not_required",
  ...
  const message = await payload.create({
    ...
    data: {
      ...
      queuedAt: receivedAt,
      aiAnalysis: {
        customerContractRequestId: requestRecord.id,
        purpose: input.request.action,
      },
    },
  });
  ...
  await updateCaseState(payload, {
    leadId: input.leadId,
    command: "customer_contract_request_received",
    idempotencyKey: `customer-contract-request:${requestRecord.id}`,
    patch: {
  ...
  await recordAuditEvent(createPayloadAuditWriter(payload), {
    ...
    metadata: {
      ...
      leadId: input.leadId,
      contractId: input.contractId,
      workOrderId: relationId(workOrder) || null,
    },
  });
  return {
