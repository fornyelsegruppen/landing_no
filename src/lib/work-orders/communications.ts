...
import type { MessageDeliveryClass } from "@/lib/messages/automation-recipient-policy";
...
export type WorkOrderCommunicationKind =
...
const scheduleKinds: WorkOrderCommunicationKind[] = [
  "schedule_confirmation",
  "reschedule_confirmation",
  "reminder_48h",
  "same_day",
...
export class CommunicationCancelledError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "CommunicationCancelledError";
  }
}
...
type JobPayload = {
  adminApprovedTransactional?: boolean;
  workOrderId: number;
...
type WorkerAssignmentJobPayload = {
  workOrderId: number;
  scheduleVersion: string;
};

type ImmediateDispatchOptions = {
  allowDuringAutomationPause?: boolean;
};
...
function parseJobPayload(value: unknown): JobPayload | null {
  ...
  if (
    ...
    typeof data.scheduleVersion !== "string" ||
    ![
      "schedule_confirmation",
      "reschedule_confirmation",
      "reminder_48h",
...
function parseWorkerAssignmentJobPayload(
  value: unknown,
): WorkerAssignmentJobPayload | null {
  if (!value || typeof value !== "object") return null;
  const data = value as Record<string, unknown>;
  if (
    typeof data.workOrderId !== "number" ||
    ...
    return null;
  return data as WorkerAssignmentJobPayload;
}

function workOrderMessageDeliveryClass(
  input: Pick<JobPayload, "adminApprovedTransactional">,
): MessageDeliveryClass {
  return input.adminApprovedTransactional ? "admin_approved" : "automation";
}
...
function communicationIsCurrent(input: JobPayload, order: CommunicationOrder) {
  if (input.kind === "completion") {
    return (
      ...
      input.scheduleVersion === `documented:${order.documentationSubmittedAt}`
    );
  }
  if (
    input.scheduleVersion !== workOrderScheduleVersion(order) ||
    !order.scheduledAt
  )
    return false;
...
async function pendingCommunicationJobs(payload: Payload, workOrderId: number) {
  const result = await payload.find({
    ...
    where: {
      and: [
        { type: { equals: "work-order.communication" } },
        { status: { in: ["pending", "retry"] } },
      ],
    },
  });
  return result.docs.filter(
    (job) => parseJobPayload(job.payload)?.workOrderId === workOrderId,
  );
}
...
export async function syncWorkOrderCommunicationJobs(
  payload: Payload,
  ...
  if (
    ...
    return { created: 0, cancelled: 0, skipped: true as const };
    ...
    if (obsolete) {
      await payload.update({
        ...
        data: {
          ...
          lastErrorMessage:
            "Planlagt kundemelding ble avbrutt fordi oppdraget ble endret.",
        },
      });
      cancelled += 1;
    }
  }
  if (!order.scheduledAt || order.status !== "scheduled")
    return { created: 0, cancelled, skipped: false as const };
  const scheduledAt = new Date(order.scheduledAt);
  if (Number.isNaN(scheduledAt.getTime()))
    throw new TypeError("Work order schedule is invalid");
  const plans: Array<[WorkOrderCommunicationKind, Date]> = [
    ["schedule_confirmation", now],
  ];
  ...
  if (process.env.WORK_ORDER_SAME_DAY_MESSAGE !== "false" && sameDay > now)
    plans.push(["same_day", sameDay]);
  for (const [kind, availableAt] of plans)
    await createJob(
      payload,
      { workOrderId: order.id, kind, scheduleVersion },
      availableAt,
      correlationId,
    );
...
export async function enqueueWorkOrderStatusCommunication(
  ...
  if (
    ...
    throw new CommunicationCancelledError(
      "Work status no longer matches the customer notification",
    );
  }
  return {
...
export async function enqueueCompletionCommunication(
  payload: Payload,
  order: {
    id: number;
    status: string;
  ...
  if (
    ...
    return { skipped: true as const };
  if (order.status !== "documented" || !order.documentationSubmittedAt)
    throw new CommunicationCancelledError(
      "Completion communication requires documented work",
    );
  return {
    skipped: false as const,
    job: await createJob(
      payload,
      {
        workOrderId: order.id,
        kind: "completion",
        scheduleVersion: `documented:${order.documentationSubmittedAt}`,
      },
      now,
      correlationId,
    ),
...
async function enqueueAdminApprovedCompletionCommunication(
  payload: Payload,
  order: {
    id: number;
    status: string;
...
export async function dispatchCompletionCommunicationNow(
  ...
  return dispatchWorkOrderCommunicationNow(
    payload,
    order,
    "completion",
    correlationId,
    provider,
    { allowDuringAutomationPause: true },
  );
}
...
export async function dispatchWorkOrderCommunicationNow(
  ...
  const queued =
    kind === "completion"
      ? options.allowDuringAutomationPause
        ? await enqueueAdminApprovedCompletionCommunication(
            payload,
            order,
            correlationId,
          )
      ...
      : statusForKind[kind]
        ? await enqueueWorkOrderStatusCommunication(
            payload,
            order,
            kind as "on_way" | "arrived" | "work_started",
            correlationId,
          )
        : {
            skipped: false as const,
  ...
  try {
    ...
    const deliveryClass = workOrderMessageDeliveryClass(
...
export async function dispatchWorkOrderRescheduleNow(
  ...
  if (!options.allowDuringAutomationPause && automaticCommunicationIsPaused()) {
    return {
      skipped: true as const,
      delivered: false as const,
      queued: false as const,
  ...
  if (!planningReason.trim())
    throw new TypeError("A rescheduling reason is required");
  if (!order.scheduledAt || order.status !== "scheduled")
    throw new CommunicationCancelledError(
      "A new complete schedule is required",
    );
  ...
  await payload.update({
    ...
    id: job.id,
    overrideAccess: true,
    data: { status: "running", attempts, startedAt: new Date().toISOString() },
  });
  try {
    const communication = await processWorkOrderCommunicationJob(
      payload,
    ...
    const deliveryClass = workOrderMessageDeliveryClass(input);
    if (provider.health().status !== "ready")
      throw new Error("Email provider requires configuration");
    await deliverMessage(
      payload,
...
export async function dispatchAdminApprovedScheduleCommunicationNow(
  payload: Payload,
  order: CommunicationOrder,
  correlationId: string,
  provider: EmailProvider = createEmailProvider(),
...
export async function dispatchAdminApprovedRescheduleCommunicationNow(
  payload: Payload,
  order: CommunicationOrder,
  previous: Pick<CommunicationOrder, "scheduledAt" | "arrivalWindow">,
  planningReason: string,
  ...
  return dispatchWorkOrderRescheduleNow(
    payload,
    order,
    previous,
    planningReason,
...
async function createWorkerAssignmentJob(
  payload: Payload,
  input: WorkerAssignmentJobPayload,
  correlationId: string,
) {
...
export async function processWorkerAssignmentNotificationJob(
  payload: Payload,
  value: unknown,
  correlationId: string,
  provider: EmailProvider = createEmailProvider(),
) {
  const input = parseWorkerAssignmentJobPayload(value);
  if (!input)
    throw new TypeError(
      "Employee notification has no valid work-order reference",
    );
  ...
  if (input.scheduleVersion !== workOrderScheduleVersion(order)) {
    throw new CommunicationCancelledError(
      "Employee assignment changed before the notification was sent",
    );
  }
  const workerId = relationId(order.assignedWorker);
  if (!workerId || !order.scheduledAt || !order.arrivalWindow) {
    throw new TypeError(
      "Complete assignment is required before notifying the employee",
    );
  }
  ...
  if (!worker.active || worker.role !== "worker" || !worker.email) {
    throw new TypeError(
      "The assigned employee account is not ready for notifications",
    );
  }
  assertControlledPilotAutomationRecipientAllowed(worker.email);
  if (provider.health().status !== "ready") {
    throw new Error("Email provider requires configuration");
  }
  const date = new Date(order.scheduledAt).toLocaleDateString("nb-NO", {
    dateStyle: "long",
    timeZone: "Europe/Oslo",
  });
  const portalUrl = `${siteConfig.url.replace(/\/$/, "")}/user/arbeid/${order.id}`;
  const text = `Hei ${worker.displayName || worker.email},\n\nDu er tildelt oppdrag ${order.reference || order.id}.\nDato: ${date}\nAvtalt ankomst: kl. ${order.arrivalWindow}\n\nÅpne medarbeiderportalen for adresse, arbeidsomfang og dokumentasjon:\n${portalUrl}\n\nEndringer vises alltid i portalen.\n\nTakfornyelse`;
  return provider.send({
    template: "worker-assignment",
    to: worker.email,
    subject: `Nytt eller oppdatert oppdrag ${order.reference || order.id}`,
    text,
    html: buildBrandedEmailHtml({
      subject: `Oppdrag ${order.reference || order.id}`,
      text,
      secureLinkLabel: "Åpne oppdraget i medarbeiderportalen",
    }),
    replyTo: siteConfig.email,
    idempotencyKey: makeIdempotencyKey("worker.assignment", input),
    correlationId,
  });
}
...
export async function notifyAssignedWorkerNow(
  ...
  const job = await createWorkerAssignmentJob(payload, input, correlationId);
...
function copyFor(
  ...
  const when = input.scheduledAt
    ? new Date(input.scheduledAt).toLocaleDateString("nb-NO", {
        dateStyle: "long",
        timeZone: "Europe/Oslo",
      })
    : "avtalt dato";
  const window = input.arrivalWindow
    ? `kl. ${input.arrivalWindow}`
    : input.scheduledAt
      ? `kl. ${new Date(input.scheduledAt).toLocaleTimeString("nb-NO", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Oslo" })}`
      : "avtalt tid";
  ...
  if (kind === "schedule_confirmation")
    return {
      category: "schedule_confirmation" as const,
      subject: "Medarbeider og tid er avtalt for takarbeidet",
      body: `Hei ${input.leadName},\n\nVi har nå tildelt en medarbeider til oppdraget ditt.\n\nDato: ${when}\nAvtalt ankomst: ${window}\n${contact}\n\nMedarbeideren varsler deg når han eller hun er på vei. Gi oss beskjed så snart som mulig dersom tidspunktet ikke passer.\n\nVennlig hilsen\nTakfornyelse\n${siteConfig.phone}`,
    };
  if (kind === "reschedule_confirmation") {
    const previousDate = input.previousScheduledAt
      ? new Date(input.previousScheduledAt).toLocaleDateString("nb-NO", {
          dateStyle: "long",
          timeZone: "Europe/Oslo",
        })
      : "tidligere avtalt dato";
    const previousWindow = input.previousArrivalWindow
      ? `kl. ${input.previousArrivalWindow}`
      : "tidligere avtalt tid";
    return {
      category: "schedule_confirmation" as const,
      subject: "Ny tid for takarbeidet",
  ...
  if (kind === "reminder_48h")
    return {
      category: "reminder" as const,
      subject: "Påminnelse om takarbeid om 48 timer",
  ...
  if (kind === "work_started")
    return {
      category: "reminder" as const,
      subject: "Takarbeidet er startet",
  ...
  return {
    ...
    subject: "Takarbeidet er fullført og dokumentert",
...
export async function processWorkOrderCommunicationJob(
  payload: Payload,
  value: unknown,
  correlationId: string,
  clock = new Date(),
  ...
  if (!input)
    throw new TypeError("Communication job has no valid work-order reference");
  const order = await payload.findByID({
    collection: "work-orders",
    id: input.workOrderId,
    depth: 0,
  ...
  if (!communicationIsCurrent(input, order))
    throw new CommunicationCancelledError(
      "Work schedule or status changed before communication was sent",
    );
  if (
    ["reminder_48h", "same_day"].includes(input.kind) &&
    order.scheduledAt &&
    clock >= new Date(order.scheduledAt)
  ) {
    throw new CommunicationCancelledError(
      "A visit reminder cannot be sent after the visit has started",
    );
  }
  ...
  if (channel !== "email")
    throw new ChannelUnavailableError(
      "Kunden har valgt SMS, men SMS-levering er ikke konfigurert. Send meldingen manuelt.",
    );
  if (!lead.email)
    throw new ChannelUnavailableError(
      "Kunden mangler e-postadresse. Velg en tilgjengelig kanal før meldingen sendes.",
    );
  if (!input.adminApprovedTransactional) {
  ...
  if (existing.docs[0])
    return { duplicate: true as const, message: existing.docs[0] };
  const workerId = relationId(order.assignedWorker);
  ...
  if (input.kind === "completion") {
    ...
    if (contractId) {
      ...
      const signedDocumentId =
        ...
        relationId(contract.signedDocument);
      if (signedDocumentId) attachments.push(signedDocumentId);
    }
    for (const photo of (order.afterPhotos || []).slice(0, 4)) {
      const id = relationId(photo);
      if (id) attachments.push(id);
    }
    const completionDocuments = await payload.find({
      ...
      where: {
        and: [
          { ownerType: { equals: "completion-certificate" } },
          { ownerId: { equals: String(order.id) } },
        ],
      },
    });
  ...
  const message = await payload.create({
    ...
    data: {
      ...
      aiAnalysis: {
        ...
        adminApprovedTransactional: Boolean(input.adminApprovedTransactional),
      },
    },
  });
  await enqueueMessageJob(
    ...
    message.id,
    correlationId,
    workOrderMessageDeliveryClass(input),
  );
