...
import { SchematicRoofEvidenceProvider, type SchematicEvidenceInput } from "./schematic-evidence";

const input = {
  address: "Lyngveien 28A, 1182 Oslo",
  addressPoint: { latitude: 59.88, longitude: 10.79 },
  candidates: [
    { id: "way/2", label: "Garasje", polygon: [{ latitude: 59.8801, longitude: 10.7902 }, { latitude: 59.8801, longitude: 10.7904 }, { latitude: 59.8799, longitude: 10.7904 }] },
    { id: "way/1", label: "Bolig", polygon: [{ latitude: 59.88, longitude: 10.7898 }, { latitude: 59.8802, longitude: 10.79 }, { latitude: 59.88, longitude: 10.7901 }] },
  ],
...
describe("schematic roof evidence", () => {
  it("renders deterministic, attributed and visibly selected evidence", async () => {
    ...
    expect(first.snapshot.candidates.map((candidate) => candidate.id)).toEqual(["way/1", "way/2"]);
  });

  it("refuses to fabricate evidence without a polygon", async () => {
    await expect(new SchematicRoofEvidenceProvider().render({ ...input, candidates: [] })).rejects.toThrow(/polygon/);
  });
});
