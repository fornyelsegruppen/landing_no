...
import { SchematicRoofEvidenceProvider, type MapEvidenceProvider } from "./schematic-evidence";
...
export async function persistSchematicMeasurementEvidence(input: {
  ...
  const evidence = await (input.provider || new SchematicRoofEvidenceProvider()).render({
  ...
  const media = await createPrivateMedia(input.payload, {
    classification: "measurement",
    ownerType: "roof-measurement",
    ownerId: String(input.measurementId),
    alt: `Skjematisk takmåling for ${input.address}`,
  }, { data: evidence.bytes, filename: evidence.filename, mimeType: evidence.mimeType });
  try {
...
export async function verifySchematicMeasurementEvidence(payload: Payload, measurement: { evidenceHash?: string | null; evidenceSnapshot?: unknown }) {
  const mediaId = typeof measurement.evidenceSnapshot === "number"
    ? measurement.evidenceSnapshot
    : measurement.evidenceSnapshot && typeof measurement.evidenceSnapshot === "object" && typeof (measurement.evidenceSnapshot as { id?: unknown }).id === "number"
      ? (measurement.evidenceSnapshot as { id: number }).id
      : null;
  if (!mediaId || !/^[a-f0-9]{64}$/i.test(measurement.evidenceHash || "")) return false;
  ...
  return createHash("sha256").update(file.data).digest("hex") === measurement.evidenceHash;
}
