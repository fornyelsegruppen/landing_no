  ...
  roofPlanes: z
    .array(
      z
        ...
        .refine(
          (plane) => plane.angleMinDegrees <= plane.angleMaxDegrees,
          "Minimum angle cannot exceed maximum angle",
        ),
    )
    .max(20),
});
    ...
    roofPlanes: {
      ...
      items: {
        ...
        properties: {
          ...
          angleMinDegrees: { type: "number" },
          angleMaxDegrees: { type: "number" },
        },
      },
    },
  },
...
export function prepareMeasurement(input: {
  ...
  const calculation = proposal.roofPlanes.length
    ? measureRoofPlanes(proposal.roofPlanes)
    : null;
