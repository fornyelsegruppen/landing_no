...
describe("Kartverket address provider", () => {
  ...
  it("normalizes punctuation and official Matrikkelen address results", async () => {
    const fetcher = async (input: RequestInfo | URL) => {
      expect(new URL(String(input)).searchParams.get("sok")).toBe("Lyngveien 28 1182 Oslo");
    ...
    await expect(provider.searchAddress(" Lyngveien 28, 1182, Oslo ")).resolves.toEqual([expect.objectContaining({
