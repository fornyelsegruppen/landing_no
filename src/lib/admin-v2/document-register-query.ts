...
const branches: readonly Branch[] = [
  ...
  },
  {
    ...
    filename: "COALESCE(NULLIF(m.filename, ''), COALESCE(NULLIF(i.reference, ''), i.id::text) || '.pdf')",
    href: "'/api/admin/media/' || m.id::text",
    status: "i.status::text",
    version: "NULL::numeric",
    createdAt: "i.created_at::timestamptz",
    hash: "i.document_hash::text",
    from: '"invoice_records" i JOIN "leads" l ON l.id = i.lead_id JOIN "private_media" m ON m.id = i.document_id',
    required: "TRUE",
  },
  {
  ...
  },
  {
    ...
    filename: "COALESCE(NULLIF(m.filename, ''), COALESCE(NULLIF(w.reference, ''), w.id::text) || '.pdf')",
    href: "'/api/admin/media/' || m.id::text",
    status: "w.status::text",
    version: "NULL::numeric",
    createdAt: "w.created_at::timestamptz",
    hash: "w.document_hash::text",
    from: '"warranties" w JOIN "leads" l ON l.id = w.lead_id JOIN "private_media" m ON m.id = w.document_id',
    required: "TRUE",
  },
];
