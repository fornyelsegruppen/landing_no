...
describe("admin list pagination", () => {
  it("preserves filters while changing only the page", () => {
    expect(
      adminListPageHref(
        "/admin-v2/cases",
        { q: "Ola Nordmann", status: "open", action: "all", page: "1" },
        2,
      ),
    ...
    expect(
      adminListPageHref(
        "/admin-v2/cases",
        { q: "Ola Nordmann", status: "open", page: "2" },
        1,
      ),
  ...
  it("normalizes invalid pages and exposes bounded navigation metadata", () => {
    ...
    expect(parseAdminListPage("4x")).toBe(1);
    expect(parseAdminListPage("999999")).toBe(10_000);
