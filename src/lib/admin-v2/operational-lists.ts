...
export type OperationalListResult = {
...
function relation(value: unknown) {
  return value && typeof value === "object" ? record(value) : undefined;
}

export async function loadOperationalList(
  payload: Pick<Payload, "find">,
  kind: OperationalListKind,
  state?: string,
): Promise<OperationalListItem[]>;
export async function loadOperationalList(
  payload: Pick<Payload, "find">,
  kind: OperationalListKind,
  state: string,
...
export async function loadOperationalList(
  ...
  state = "all",
  pagination?: AdminListPagination,
): Promise<OperationalListItem[] | OperationalListResult> {
  ...
  if (kind === "offers") {
    ...
    activeFilter = {
      and: [
        { "lead.recordState": { equals: "active" } },
    ...
    activeFilter = {
      and: [
        { "quote.lead.recordState": { equals: "active" } },
        { status: { not_equals: "superseded" } },
      ],
    };
  } else {
    collection = "work-orders";
    activeFilter = { "lead.recordState": { equals: "active" } };
  }
  const statusFilter: Where | null =
    state === "all"
      ? null
      : kind === "work" && state === "active"
        ? {
            status: {
              in: [
                "scheduled",
        ...
        : kind === "work" && state === "finished"
          ? { status: { in: ["completed", "documented", "cancelled"] } }
          : { status: { equals: state } };
  const result = await payload.find({
    ...common,
    collection,
    where: statusFilter ? { and: [activeFilter, statusFilter] } : activeFilter,
  });
  const items = result.docs.map((raw) => {
    ...
    return {
      ...
      detail:
        kind === "work"
          ? text(item.workSummary)
          : text(item.serviceDescription),
      employee:
        kind === "work"
          ? text(relation(item.assignedWorker)?.displayName) ||
            text(relation(item.assignedWorker)?.name)
          : undefined,
  ...
  const fallback = adminListPaginationMeta(totalDocs, page);
  return {
    items,
    hasNextPage: result.hasNextPage ?? fallback.hasNextPage,
    hasPrevPage: result.hasPrevPage ?? fallback.hasPrevPage,
    page: result.page || fallback.page,
    totalDocs,
    totalPages: result.totalPages ?? fallback.totalPages,
  };
}
