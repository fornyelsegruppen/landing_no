...
beforeAll(async () => {
  ...
  await database.exec(`
    ...
    INSERT INTO leads (id, status) VALUES (1, 'quoted'), (2, 'quoted');
...
describe("loadCaseCurrentSelection", () => {
  it("selects current IDs beyond capped source pages and preserves effective signed history", async () => {
    const result = await loadCaseCurrentSelection(payloadForDatabase(), admin, 1);
  ...
  it("uses quote status priority and ID tie-breakers for equal versions", async () => {
    const result = await loadCaseCurrentSelection(payloadForDatabase(), admin, 2);
