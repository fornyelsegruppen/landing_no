...
describe("quote PDF", () => {
  it("preserves professional Norwegian units and letters", () => {
    expect(pdfSafe("Målt takareal 102,3 m² · øvre grense")).toBe(
      "Målt takareal 102,3 m² | øvre grense",
    );
  });
  ...
  it("builds a branded final contract with both signatures in the document", async () => {
    const quote = buildQuoteSnapshot({
      quoteReference: "T-2-V1",
      leadId: 2,
      serviceKey: "takvask",
      serviceDescription: "Takvask",
      propertyAddress: "Testveien 2",
      measurement: {
        id: 2,
        version: 1,
        inputHash: "a".repeat(64),
    ...
    const contract = buildContractSnapshot({
      contractReference: "K-2-V1",
      quote,
      customer: { name: "Test Kunde", address: "Testveien 2" },
      terms: {
        version: "legal-v1",
        text: "Avtalevilkår.",
    ...
    const companyEvidence = createCompanySignatureEvidence({
      contract,
      expectedDocumentHash: documentHash(contract),
      signatureData,
      signerName: "Kari Admin",
      signerUserId: 1,
      ipAddress: "",
      userAgent: "",
      securitySalt: "s".repeat(32),
    ...
    const bytes = await buildQuoteContractPdf({
      ...
      signatureData,
      evidence,
      companySignatureData: signatureData,
      companyEvidence,
    });
    const pdf = await PDFDocument.load(bytes);
  ...
  it("requires and embeds the immutable visual evidence for a schematic v2 snapshot", async () => {
    ...
    const quote = buildQuoteSnapshot({
      quoteReference: "T-3-V1",
      leadId: 3,
      serviceKey: "takvask",
      serviceDescription: "Takvask",
      propertyAddress: "Testveien 3",
      measurement: {
        id: 3,
        version: 2,
        inputHash: "a".repeat(64),
        ...
        assumptions: ["Kontrolleres på stedet"],
        mode: "schematic",
        buildingIdentifier: "way/3",
        evidenceMediaId: 33,
        evidenceHash: "c".repeat(64),
        evidenceAttribution: "© OpenStreetMap contributors",
        angleMinDegrees: 22,
        angleMaxDegrees: 32,
      },
      pricing: {
    ...
    const contract = buildContractSnapshot({
      contractReference: "K-3-V1",
      quote,
      customer: { name: "Test Kunde", address: "Testveien 3" },
      terms: {
        version: "legal-v1",
        text: "Avtalevilkår.",
        withdrawalInstructions: "Informasjon om angrerett.",
    ...
    await expect(buildQuoteContractPdf({ contract })).rejects.toThrow(
