...
import {
  appendTimeline,
  loadAuthorizedWorkOrder,
  relationId,
} from "@/lib/work-orders/access";
import {
  assessAcceptedChangePrecheck,
  assessPrecheck,
} from "@/lib/work-orders/precheck";
...
const precheckSchema = z
  ...
  .superRefine((value, context) => {
    if (value.safetyStatus === "blocked" && value.safetyNotes.length < 5)
      context.addIssue({
        code: "custom",
        path: ["safetyNotes"],
        message: "HMS-avvik må beskrives",
      });
    if (value.scopeChanged && value.scopeChangeDetails.length < 5)
      context.addIssue({
        code: "custom",
        path: ["scopeChangeDetails"],
        message: "Omfangsavvik må beskrives",
      });
  });
const documentationSchema = z.object({
  action: z.literal("submit_documentation"),
  afterPhotoIds: z.array(z.number().int().positive()).min(2).max(20),
  completionNotes: z.string().trim().min(10).max(4_000),
});
...
const allowedStatusesByAction = {
  on_way: ["scheduled"],
  arrive: ["on_way"],
  begin_precheck: ["arrived", "blocked"],
...
function assertActionAllowed(
  status: string,
  action: keyof typeof allowedStatusesByAction,
) {
  const allowed = allowedStatusesByAction[action] as readonly string[];
  if (!allowed.includes(status)) {
    throw new Error(
      "Handlingen kan ikke utføres i oppdragets nåværende status",
    );
  }
}

async function verifyWorkMedia(
  ...
  const result = await payload.find({
    ...
    where: {
      and: [
        { id: { in: uniqueIds } },
        { classification: { equals: "work" } },
        { ownerType: { equals: "work-order" } },
        { ownerId: { equals: String(orderId) } },
      ],
    },
  });
  if (result.docs.length !== uniqueIds.length)
    throw new Error("One or more work photos are invalid");
  return uniqueIds;
}
  ...
  try {
    ...
    if (!parsed.success)
      return NextResponse.json(
        {
          error: "Ugyldig eller mangelfull registrering",
    ...
    if (
      ...
      order.blockingReasons.includes("CUSTOMER_CANCELLATION_REQUEST")
    ) {
      return NextResponse.json(
        {
          code: "customer_cancellation_pending",
          error:
            "Work is paused while the customer's cancellation request is being reviewed.",
        },
        { status: 409 },
      );
    }
    if (
      ...
      ["begin_precheck", "submit_precheck"].includes(parsed.data.action) &&
      order.priceOutcome !== "hms_blocked" &&
      !relationId(order.approvedChangeAgreement)
    ) {
      return NextResponse.json(
        {
          code: "change_agreement_required",
          error:
            "An accepted written change agreement is required before the precheck can be repeated.",
        },
        { status: 409 },
      );
    }
    ...
    if (parsed.data.action === "on_way") {
      ...
      const afterPhotoIds = await verifyWorkMedia(
        payload,
        order.id,
        parsed.data.afterPhotoIds,
      );
      data = {
        status: "completed",
        afterPhotos: afterPhotoIds,
        completionNotes: parsed.data.completionNotes,
        documentationSubmittedAt: now,
      };
      changedFields = [
        "afterPhotos",
        "completionNotes",
      ...
      if (parsed.data.action !== "submit_precheck")
        throw new Error("Unsupported work-order action");
      const beforePhotoIds = await verifyWorkMedia(
        payload,
        order.id,
        parsed.data.beforePhotoIds,
      );
      const quote = await payload.findByID({
        collection: "quotes",
        id: relationId(order.quote)!,
        depth: 0,
        overrideAccess: true,
      });
      ...
      const rule = await payload.findByID({
        ...
        id: snapshot.pricing.ruleId,
        depth: 0,
        overrideAccess: true,
      });
      if (
        rule.status !== "approved" ||
        rule.version !== snapshot.pricing.ruleVersion ||
        rule.unitPriceExVatOre !== snapshot.pricing.unitPriceExVatOre ||
        rule.vatBasisPoints !== snapshot.pricing.vatBasisPoints
      ) {
        throw new Error(
          "The signed price rule no longer matches the calculation basis",
        );
      }
      ...
      if (acceptedChangeId) {
        const acceptedChange = await payload.findByID({
          collection: "change-agreements",
          id: acceptedChangeId,
          depth: 0,
          overrideAccess: true,
        });
        const changeSnapshot = changeAgreementSnapshotSchema.parse(
          acceptedChange.snapshot,
        );
        if (
          acceptedChange.status !== "accepted" ||
          changeSnapshot.workOrderId !== order.id ||
          changeSnapshot.contractDocumentHash !== order.contractDocumentHash
        )
          throw new Error("The approved change agreement is invalid");
        assessment = assessAcceptedChangePrecheck({
          actualAreaTenths: parsed.data.actualAreaTenths,
          agreedAreaTenths: changeSnapshot.after.areaTenths,
          agreedSubtotalExVatOre: changeSnapshot.after.subtotalExVatOre,
          agreedTotalIncVatOre: changeSnapshot.after.totalIncVatOre,
          unitPriceExVatOre: rule.unitPriceExVatOre,
          vatBasisPoints: rule.vatBasisPoints,
          hmsSafe: parsed.data.safetyStatus === "safe",
          scopeChangedAgain: parsed.data.scopeChanged,
        });
      }
      data = {
    ...
    data.eventTimeline = appendTimeline(order.eventTimeline, {
    ...
    await recordAuditEvent(createPayloadAuditWriter(payload), {
      actorId: Number(user.id),
      action: `work-order.${parsed.data.action}`,
    ...
    const communicationKind =
      parsed.data.action === "on_way"
        ? "on_way"
        : parsed.data.action === "arrive"
          ? "arrived"
    ...
    if (communicationKind) {
      try {
        const notification = await dispatchWorkOrderCommunicationNow(
          payload,
          updated,
          communicationKind,
          correlationId,
        );
        customerNotification = notification.delivered
          ? "sent"
          : notification.queued
            ? "queued"
            : "skipped";
        ...
        captureException(error, {
          route: "POST /api/worker/work-orders/[id]",
    ...
    return NextResponse.json({
      ok: true,
      status: updated.status,
      decision: updated.precheckDecision,
      priceOutcome: updated.priceOutcome,
      blockingReasons: updated.blockingReasons,
      actualTotalIncVatOre: updated.actualTotalIncVatOre,
