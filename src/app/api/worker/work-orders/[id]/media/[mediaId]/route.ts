...
export async function GET(request: Request, context: { params: Promise<{ id: string; mediaId: string }> }) {
  ...
  if (!/^\d+$/.test(id) || !/^\d+$/.test(mediaId)) return emptyPrivateResponse(404);
  ...
  if (!media || media.classification !== "work" || media.ownerType !== "work-order" || media.ownerId !== String(order.id) || !media.mimeType?.startsWith("image/")) return emptyPrivateResponse(404);
  const file = await readPrivateMediaContent(media);
  return new NextResponse(file.data, { headers: { "Content-Type": file.contentType, ...workerPrivateNoStoreHeaders, "X-Content-Type-Options": "nosniff" } });
}
