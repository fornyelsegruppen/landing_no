...
const snapshot = buildQuoteSnapshot({
  ...
  pricing: {
    ...
    ruleId: 6,
    ruleVersion: 2,
    unitPriceExVatOre: 12500,
    subtotalExVatOre: 1375000,
    vatBasisPoints: 2500,
    vatOre: 343750,
    totalIncVatOre: 1718750,
    toleranceBasisPoints: 1000,
    maximumTotalIncVatOre: 1890625,
  },
  termsVersion: "v1",
...
function request(body: Record<string, unknown>) {
  return new Request("http://localhost/api/worker/work-orders/9", {
    method: "POST",
...
describe("worker work-order API", () => {
  beforeEach(() => {
    mocks.auth
      .mockReset()
      .mockResolvedValue({ user: { id: 2, role: "worker", active: true } });
  ...
  it("allows only the assigned worker to advance the order", async () => {
    ...
    const response = await POST(request({ action: "on_way" }), {
    ...
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        data: expect.objectContaining({ status: "on_way" }),
      }),
    );
  });
  ...
  it("allows a non-cancellation blocker to restart the precheck", async () => {
    const order = {
      id: 9,
      assignedWorker: 2,
      status: "blocked",
      blockingReasons: ["Kontrollmålt pris overstiger avtalt maksimalbeløp."],
      approvedChangeAgreement: 8,
      priceOutcome: "over_maximum",
      eventTimeline: [],
    };
    mocks.findByID.mockResolvedValueOnce(order);
    mocks.update.mockImplementation(
    ...
    const response = await POST(request({ action: "begin_precheck" }), {
      params: Promise.resolve({ id: "9" }),
    });

    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ status: "precheck" });
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({
        collection: "work-orders",
        id: 9,
        data: expect.objectContaining({ status: "precheck" }),
      }),
    );
  });

  it("requires an accepted change agreement before a price-blocked retry", async () => {
    mocks.findByID.mockResolvedValueOnce({
      id: 9,
      assignedWorker: 2,
      status: "blocked",
  ...
  it.each(["begin_precheck", "submit_precheck"])(
    "keeps an unresolved customer cancellation frozen for %s",
    async (action) => {
      mocks.findByID.mockResolvedValueOnce({
        id: 9,
        assignedWorker: 2,
      ...
      const body =
        action === "begin_precheck"
          ? { action }
          : {
              action,
              beforePhotoIds: [11, 12],
              roofType: "teglstein",
              actualAreaTenths: 1000,
              measurementMethod: "laser",
              slopeBasis: "Målt 32 grader",
              visibleCondition: "Takflaten er kontrollert",
              safetyStatus: "safe",
              safetyNotes: "",
              scopeChanged: false,
              scopeChangeDetails: "",
