...
import {
  prepareMeasurement,
  roofProposalSchema,
} from "@/lib/measurements/proposal";
import { correlationIdFromHeaders } from "@/lib/observability/correlation-id";
import { getPayload } from "@/lib/payload";
import {
  KartverketAddressProvider,
  norgeIBilderAccess,
} from "@/lib/providers/kartverket-address-provider";
import {
...
const createSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    action: z.literal("create_from_candidate"),
    leadId: z.number().int().positive(),
    expectedRevision: z.number().int().positive().optional(),
    addressId: z.string().min(1),
    buildingId: z.string().min(1),
    slopeDegrees: z.union([
      z.literal(22),
      z.literal(27),
      z.literal(32),
      z.literal(36),
  ...
  z.object({
    ...
    reason: z.string().trim().min(5).max(500),
    confirmLargeDeviation: z.boolean().default(false),
  }),
]);
...
async function createVisualMeasurement(input: {
  ...
  imagerySourceUrl?: string;
  lead: { id: number; inquiryType?: string | null };
  leadId: number;
  license: string;
  mapImageId?: number;
  payload: Payload;
  proposal: z.infer<typeof roofProposalSchema>;
  expectedRevision?: number;
  requireApprovedPriceRule: boolean;
}) {
  const rules = await input.payload.find({
    ...
    where: {
      and: [
        { serviceKey: { equals: input.lead.inquiryType } },
  ...
  if (!prepared.calculation)
    throw new TypeError(
      `Measurement cannot be calculated: ${prepared.gate.reasons.join(", ")}`,
    );
  ...
  const measurement = await input.payload.create({
    ...
    overrideAccess: true,
    data: {
      reference: `TM-${input.leadId}-V${version}`,
      lead: input.leadId,
      version,
      supersedes: prior?.id,
  ...
  await updateCaseState(input.payload, {
    leadId: input.leadId,
    actorId: input.actorId,
    expectedRevision: input.expectedRevision,
    command: "measurement_candidate_selected",
    idempotencyKey: `measurement-candidate:${measurement.id}`,
    patch: {
      status: "measuring",
      nextActionOwner: "administrator",
      nextActionBlocker: null,
      nextAction: "Kontroller og godkjenn valgt bygning og takmåling.",
...
export async function GET(request: Request) {
  ...
  if (Number.isInteger(leadId) && leadId > 0) {
    const lead = await auth.payload.findByID({
      collection: "leads",
      id: leadId,
      depth: 0,
      overrideAccess: true,
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            "Building service is temporarily unavailable. Correct the case manually or try again.",
          code: "BUILDING_SERVICE_UNAVAILABLE",
          addresses,
          selectedAddress,
          candidates: [],
        },
    ...
    return NextResponse.json({
      ...
      selectedAddress,
      candidates,
      imagery: norgeIBilderAccess(),
    });
  }
  const query = params.get("query") ?? "";
  const addresses =
    query.length >= 4
      ? await new KartverketAddressProvider().searchAddress(query)
      : [];
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Invalid measurement data", details: parsed.error.flatten() },
      { status: 400 },
    );
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Case was changed by another administrator. Refresh before saving.",
  ...
  if (parsed.data.action !== "create") {
    ...
    if (activeQuotes.docs[0] && activeQuotes.docs[0].status !== "draft") {
      return NextResponse.json(
        {
          error:
            "An issued or accepted quote requires a controlled change agreement",
          code: "CONTROLLED_CHANGE_REQUIRED",
        },
        { status: 409 },
      );
    }
  ...
  if (parsed.data.action === "create_manual") {
    ...
    const differencePercent = manualAreaDeviationPercent(
      priorArea || undefined,
      areaTenths,
    );
    if (differencePercent > 20 && !parsed.data.confirmLargeDeviation)
      return NextResponse.json(
        {
          error: "Manual area differs by more than 20%",
    ...
    const snapshot = {
      ...
      manualOverride: {
        areaTenths,
        source: parsed.data.manualAreaSource,
        reason: parsed.data.reason,
        overriddenBy: user.id,
        overriddenAt: now,
      },
      formula:
        "Godkjent manuelt takareal brukes som prisgrunnlag; arealet kontrolleres på stedet før arbeid.",
    };
    const measurement = await payload.create({
      collection: "roof-measurements",
      overrideAccess: true,
      data: {
        reference: `TM-${lead.id}-V${version}`,
        lead: lead.id,
        version,
        supersedes: prior?.id,
        ...
        normalizedAddress:
          uniqueAddressParts([
            ...
            lead.postal,
            lead.city,
          ]).join(" ") || "Adresse ikke verifisert",
        source: "Manuell arealregistrering",
        license: "Ikke aktuelt",
        credits: "Ingen visuell kilde",
        imageryLicensed: false,
        capturedAt: now,
        roofPlanes: [],
        horizontalAreaTenths: areaTenths,
        actualAreaMinTenths: areaTenths,
        actualAreaMaxTenths: areaTenths,
        calculationSnapshot: snapshot,
        inputHash: measurementSnapshotHash({ ...snapshot, version }),
        confidence: "medium",
        confidenceReasoning: `Manuelt areal: ${parsed.data.reason}`,
        status: "review_required",
        blockingReasons: [],
        manualAreaSource: parsed.data.manualAreaSource,
        manualAreaReason: parsed.data.reason,
      },
    });
    try {
      await updateCaseState(payload, {
        ...
        command: "manual_no_visual_measurement",
        idempotencyKey: `manual-no-visual:${measurement.id}`,
        patch: {
          status: "measuring",
          nextActionOwner: "administrator",
          nextActionBlocker: null,
          nextAction: "Kontroller og godkjenn manuelt takareal uten kart.",
          nextActionAt: now,
        },
      });
    } catch (error) {
    ...
    await recordAuditEvent(createPayloadAuditWriter(payload), {
      actorId: user.id,
      action: "measurement.manual-no-visual-created",
      ...
      metadata: { differencePercent, supersedes: prior?.id },
    });
    if (!commercialPackageEnabled) {
      return NextResponse.json(
        { measurement, differencePercent, package: null },
        { status: 201 },
      );
    }
    const packageResult = await createPreparedPackageForMeasurement(payload, {
      leadId: lead.id,
      measurementId: measurement.id,
    });
    return NextResponse.json(
      {
        ...
        differencePercent,
        package: {
          calculationId: packageResult.calculation.id,
          quoteId: packageResult.quote.id,
          contractId: packageResult.contract.id,
        },
      },
      { status: 201 },
    );
  ...
  if (parsed.data.action === "create_from_candidate") {
    ...
    if (!building)
      return NextResponse.json(
        { error: "Building candidate is no longer available" },
        { status: 409 },
      );
    ...
    result = await createVisualMeasurement({
      ...
      proposal: {
        ...
        confidenceReasoning: `${building.confidenceReasoning} Administrator valgte bygget og vinkelgruppen ${slopeDegrees}°: ${parsed.data.reason}`,
        roofPlanes: [
          {
            id: `${building.id}-roof`,
      ...
      imagerySource: building.source,
      imagerySourceUrl: building.sourceUrl,
      license: building.license,
      credits: building.credits,
      requireApprovedPriceRule,
    });
  } else {
    const selectedBuildingId =
      parsed.data.proposal.buildingIdentifier || "manual-building";
    const firstPolygon = parsed.data.proposal.roofPlanes[0]?.polygon;
    if (!firstPolygon?.length)
      return NextResponse.json(
        { error: "Selected building polygon is required" },
        { status: 409 },
      );
    const candidates = [
      {
        ...
        horizontalAreaSquareMeters: 0,
        distanceToAddressMeters: 0,
        containsAddress: true,
        confidence: parsed.data.proposal.confidence,
        confidenceReasoning: parsed.data.proposal.confidenceReasoning,
        source: "OpenStreetMap building footprint via Overpass API",
        sourceUrl:
          parsed.data.imagerySourceUrl || "https://www.openstreetmap.org",
    ...
    result = await createVisualMeasurement({
      ...
      imagerySource: parsed.data.imagerySource,
      imagerySourceUrl: parsed.data.imagerySourceUrl,
      license: parsed.data.license,
      credits: parsed.data.credits,
      mapImageId: parsed.data.mapImageId,
      requireApprovedPriceRule,
    });
  }
  await recordAuditEvent(createPayloadAuditWriter(payload), {
    actorId: user.id,
    action: "measurement.building-selected",
    entityType: "roof-measurement",
  ...
  if (!commercialPackageEnabled) {
    return NextResponse.json(
      { measurement: result.measurement, gate: result.gate, package: null },
      { status: 201 },
    );
  }
  const packageResult = await createPreparedPackageForMeasurement(payload, {
