...
export async function POST(
  ...
  let companySignatureMedia: Awaited<
    ReturnType<typeof createPrivateMedia>
  > | null = null;
  let finalDocumentMedia: Awaited<
    ReturnType<typeof createPrivateMedia>
  > | null = null;
  ...
  try {
    ...
    if (contract.status !== "signed")
      throw new TypeError(
        "The customer must sign before the supplier can sign",
      );
    ...
    if (quote.status !== "accepted")
      throw new TypeError("The customer has not accepted the quote");
    const snapshot = contract.snapshot as unknown as ContractSnapshot;
    const customerEvidence =
      contract.signatureEvidence as unknown as SignatureEvidenceRecord | null;
    assertCustomerSignatureProof(contract);
    if (!customerEvidence)
      throw new TypeError("Customer signature evidence is missing");
    ...
    const customerSignatureImageId = relationId(
      contract.customerSignatureImage,
    );
    if (customerSignatureImageId) {
      const media = await payload.findByID({
        collection: "private-media",
        id: customerSignatureImageId,
        depth: 0,
        overrideAccess: true,
      });
      ...
      if (legacySignedDocumentId) {
        const legacyDocument = await payload.findByID({
          collection: "private-media",
          id: legacySignedDocumentId,
          depth: 0,
          overrideAccess: true,
        });
        ...
        if (extracted)
          customerSignatureData = `data:image/png;base64,${extracted.toString("base64")}`;
      }
    }
    if (!customerSignatureData)
      throw new TypeError(
        "Customer signature image is unavailable. Reissue the contract for customer signing before counter-signing.",
      );
    ...
    companySignatureMedia = await createPrivateMedia(
      payload,
      {
    ...
    finalDocumentMedia = await createPrivateMedia(
      payload,
      {
        classification: "contract",
        ownerType: "contract",
    ...
    await updateCaseState(payload, {
      leadId,
      actorId: user.id,
      command: "company_countersigned",
      idempotencyKey: `company-countersign:${contract.id}`,
      patch: {
        status: "converted",
        nextActionOwner: "administrator",
        nextAction:
          "Kontrakten er signert av begge parter. Opprett og planlegg arbeidsordre.",
        nextActionAt: evidence.signedAt,
      },
    });
      ...
      if (provider.health().status === "ready") {
        try {
          ...
          captureException(error, {
            route: "POST /api/admin/contracts/[id]/sign",
            operation: "final-contract-delivery",
            correlationId,
          });
        }
    ...
    return NextResponse.json({
      ok: true,
      status: "fully_signed",
      documentId: finalDocumentMedia.id,
      notification,
    });
  } catch (error) {
    if (!contractCommitted && finalDocumentMedia)
      await deletePrivateMedia(payload, finalDocumentMedia).catch(
        () => undefined,
      );
    if (!contractCommitted && companySignatureMedia)
      await deletePrivateMedia(payload, companySignatureMedia).catch(
        () => undefined,
      );
    captureException(error, {
      route: "POST /api/admin/contracts/[id]/sign",
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Contract signing failed",
        ...(error instanceof StaleCommercialContextError
          ? {
              code: "STALE_COMMERCIAL_CONTEXT",
              currentReference: error.currentReference,
            }
          : {}),
        correlationId,
      },
