...
import {
  approveAndSendPreparedLeadPackage,
...
const actionSchema = z
  .discriminatedUnion("action", [
    ...
    z.object({
      action: z.literal("update_intake"),
...
export async function POST(
  ...
  try {
    ...
    if (parsed.data.action === "generate_reply") {
      ...
      const generated = await createLeadAiReply(
        payload,
      ...
      result = await approveAndSendPreparedLeadPackage(
        payload,
        leadId,
        user.id,
        correlationId,
      ...
      result = {
        polished: {
          subject: polished.result.subject,
          bodyText: polished.result.replyDraft,
        },
      };
      ...
      if (relationId(message.lead) !== leadId || message.status !== "draft") {
        throw new TypeError(
          "Only a draft in this customer case can be cancelled",
        );
      }
      ...
      if (relationId(message.lead) !== leadId)
        return NextResponse.json(
      ...
      if (lead.nextActionBlocker !== "CUSTOMER_CANCELLATION_REQUEST")
        throw new TypeError("This case has no pending cancellation request");
      const now = new Date().toISOString();
      const workOrders = await payload.find({
      ...
      if (
        ...
        Array.isArray(workOrder.blockingReasons) &&
        workOrder.blockingReasons.includes("CUSTOMER_CANCELLATION_REQUEST")
      ) {
        const previousStatus = workOrder.statusBeforeCustomerCancellation;
        const restored: "unassigned" | "assigned" | "scheduled" | "on_way" =
          previousStatus &&
          ["unassigned", "assigned", "scheduled", "on_way"].includes(
            previousStatus,
          )
            ? (previousStatus as
                "unassigned" | "assigned" | "scheduled" | "on_way")
            : "unassigned";
        await payload.update({
          collection: "work-orders",
          id: workOrder.id,
          ...
          data: {
            ...
            blockingReasons: workOrder.blockingReasons.filter(
              (item) => item !== "CUSTOMER_CANCELLATION_REQUEST",
            ),
      ...
      let sourceMessageId = workOrder
        ? relationId(workOrder.cancellationRequestMessage)
        : null;
      if (!sourceMessageId) {
        const cancellationMessages = await payload.find({
          collection: "messages",
          depth: 0,
          limit: 1,
          sort: "-createdAt",
      ...
      const confirmation = await payload.create({
        collection: "messages",
        overrideAccess: true,
        data: {
          lead: leadId,
          ...
          subject:
            parsed.data.decision === "cancel"
              ? "Avklaring av kanselleringsforespørselen"
          ...
          status: "draft",
          idempotencyKey: makeIdempotencyKey("cancellation.resolution", {
            decision: parsed.data.decision,
            leadId,
            revision: lead.caseRevision,
          }),
          aiAssisted: false,
          aiAnalysis: {
            cancellationDecision: parsed.data.decision,
            administratorReason: parsed.data.reason,
            sourceMessageId,
          },
        },
      });
      await updateCaseState(payload, {
        leadId,
        actorId: user.id,
        command: "resolve_cancellation",
        idempotencyKey: `${correlationId}:resolve-cancellation`,
        patch:
          parsed.data.decision === "cancel"
            ? {
                status: "closed",
                ...
                nextAction:
                  "Kontroller og send kanselleringsbekreftelsen. Arkiver deretter saken med korrekt klassifisering.",
                nextActionAt: now,
                closedAt: now,
              }
            : {
                ...
                nextActionOwner: "administrator",
                nextActionBlocker: null,
                nextAction: workOrder
                  ? "Kontroller gjenopptatt arbeidsplan og informer kunden."
                  : "Opprett arbeid når begge parter har signert.",
                nextActionAt: now,
                closedAt: null,
              },
      });
      ...
      const key = makeIdempotencyKey("lead.information-request", {
        leadId,
        minute: new Date().toISOString().slice(0, 16),
      });
      const missingNo = [
        !lead.address || lead.address === "Ikke oppgitt"
          ? "nøyaktig adresse til boligen"
          : null,
        !lead.approxSqm ? "omtrentlig takareal dersom du kjenner det" : null,
        !lead.photoUrls?.trim()
          ? "oversiktsbilder av taket tatt trygt fra bakken"
          : null,
      ].filter(Boolean);
      const english = lead.language === "en";
      const missing = missingNo.length
        ? missingNo.join(", ")
        : "en kort beskrivelse av takets tilstand og hva du ønsker hjelp med";
      const message = await payload.create({
        ...
        data: {
          ...
          subject: english
            ...
            : "Vi trenger noen flere opplysninger",
          bodyText: english
      ...
      const updated = await updateCaseState(payload, {
        leadId,
        actorId: user.id,
        expectedRevision: parsed.data.expectedRevision,
        command: "update_intake",
        idempotencyKey: `${correlationId}:update-intake`,
        patch: {
      ...
      await updateCaseState(payload, {
        ...
        idempotencyKey: `${correlationId}:start-measurement`,
        patch: {
          status: "measuring",
          nextActionOwner: "administrator",
      ...
      await updateCaseState(payload, {
        ...
        actorId: user.id,
        command: "close",
        idempotencyKey: `${correlationId}:close`,
        patch: {
          status: "closed",
          nextActionOwner: "administrator",
          nextAction: null,
    ...
    return NextResponse.json(
      {
        error:
          error instanceof TypeError ? error.message : "Lead action failed",
        correlationId,
      },
