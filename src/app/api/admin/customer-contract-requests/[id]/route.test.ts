...
vi.mock("@/lib/cases/case-command", () => ({
  updateCaseState: mocks.updateCase,
}));
vi.mock("@/lib/contracts/contract-change-package", () => ({
  prepareContractChangePackage: mocks.prepareChange,
}));
...
function request(body: Record<string, unknown>) {
  return new Request(
    "http://localhost/api/admin/customer-contract-requests/10",
    {
      method: "POST",
...
describe("administrator customer contract request decision", () => {
  beforeEach(() => {
    mocks.auth
      .mockReset()
      .mockResolvedValue({ user: { id: 3, role: "admin", active: true } });
    mocks.findByID
      .mockReset()
      .mockImplementation(async ({ collection }: { collection: string }) => {
        if (collection === "customer-contract-requests")
          return {
            id: 10,
            reference: "ANG-2-TEST",
            lead: 2,
            kind: "withdrawal",
            status: "admin_review",
            sourceMessage: 9,
        ...
        if (collection === "leads")
          return { id: 2, name: "Test Kunde", email: "test@example.test" };
      ...
      docs: [
        {
          id: 7,
          status: "blocked",
          statusBeforeCustomerCancellation: "scheduled",
          blockingReasons: ["CUSTOMER_CANCELLATION_REQUEST"],
        },
      ],
    });
    mocks.update
    ...
    mocks.create
      ...
      .mockImplementation(
        async ({
          ...
          collection: string;
          data: Record<string, unknown>;
        }) => ({ id: collection === "messages" ? 20 : 21, ...data }),
      );
    mocks.enqueue.mockReset().mockResolvedValue({ id: 30, status: "pending" });
    mocks.deliver.mockReset().mockResolvedValue({
      duplicate: false,
      message: { id: 20, status: "sent" },
    });
    mocks.prepareChange.mockReset().mockResolvedValue({
      quote: { id: 31, reference: "T-2-V2" },
      contract: { id: 41, reference: "K-2-V2" },
      sourceQuote: { id: 11 },
      duplicate: false,
    });
  ...
  it("closes the order, clears the work hold and automatically sends the customer confirmation", async () => {
    const internalDecision =
      "Sutartis ir darbų būsena patikrinta administratoriaus.";
    const response = await POST(
      request({ decision: "close", reason: internalDecision }),
    ...
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        data: expect.objectContaining({
          status: "closed",
          reviewedBy: 3,
          administratorDecision: internalDecision,
        }),
      }),
    );
      ...
      expect.objectContaining({
        ...
        data: expect.objectContaining({
          ...
          bodyText: expect.stringContaining(
            "bekrefter at avtalen er avsluttet",
          ),
    ...
    const customerMessage = mocks.create.mock.calls.find(
  ...
  it("keeps work frozen while a consented follow-up is scheduled", async () => {
    const response = await POST(
      request({
        ...
        reason: "Kunden ønsker kontakt på valgt dato.",
        followUpAt: "2026-11-26T10:00:00.000Z",
      }),
      { params: Promise.resolve({ id: "10" }) },
    );
    expect(response.status).toBe(200);
    expect(mocks.update).not.toHaveBeenCalledWith(
      expect.objectContaining({ collection: "work-orders" }),
    );
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({
        collection: "customer-contract-requests",
  ...
  it("prepares a complete controlled replacement package without sending a holding email", async () => {
    ...
    expect(response.status).toBe(200);
    expect(mocks.prepareChange).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        administratorId: 3,
    ...
    expect(mocks.update).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        data: expect.objectContaining({
          ...
          followUpOutcome: expect.stringContaining("T-2-V2"),
        }),
      }),
    );
    expect(mocks.updateCase).toHaveBeenCalledWith(
