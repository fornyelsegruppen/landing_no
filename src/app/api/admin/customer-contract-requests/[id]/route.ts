...
const schema = z
  ...
  .superRefine((value, context) => {
    ...
    if (value.decision === "alternative" && !value.targetServiceKey) {
      context.addIssue({
        code: "custom",
        message: "Replacement service is required",
        path: ["targetServiceKey"],
      });
    }
  });
...
export async function POST(
  ...
  try {
    ...
    const contractRequest = await payload.findByID({
    ...
    if (
      ["closed", "recovered", "do_not_contact"].includes(contractRequest.status)
    ) {
      return NextResponse.json({
        ok: true,
    ...
    const closes =
      ...
      parsed.data.decision === "do_not_contact";
    const resumes = parsed.data.decision === "continue";
    if (closes && !lead.email) {
      throw new TypeError(
        "Customer email is required before the closure can be confirmed",
      );
    }

    let revisedPackage:
      Awaited<ReturnType<typeof prepareContractChangePackage>> | undefined;
    if (parsed.data.decision === "alternative") {
      if (workOrder)
        throw new TypeError(
          "A started work flow must be changed through a controlled work change agreement",
        );
      ...
      const staleHoldingDrafts = await payload.find({
        collection: "messages",
        depth: 0,
        limit: 20,
        overrideAccess: true,
      ...
      for (const draft of staleHoldingDrafts.docs) {
        const analysis =
          draft.aiAnalysis && typeof draft.aiAnalysis === "object"
            ? (draft.aiAnalysis as Record<string, unknown>)
            : {};
        if (
          analysis.customerContractRequestId === contractRequest.id &&
          analysis.decision === "alternative"
        ) {
    ...
    const status =
      parsed.data.decision === "close"
        ...
        : parsed.data.decision === "continue"
          ? "recovered"
          : parsed.data.decision === "alternative"
            ? "alternative_requested"
            : parsed.data.decision === "schedule_follow_up"
              ? "follow_up_scheduled"
              : "do_not_contact";
    await payload.update({
      collection: "customer-contract-requests",
      ...
      data: {
        ...
        ...(closes ? { closedAt: now } : {}),
        ...(parsed.data.decision === "do_not_contact"
          ? { recoveryPotential: "red" }
          : {}),
        ...(parsed.data.decision === "continue"
          ? {
              recoveryPotential: "green",
              followUpOutcome:
                "Avtalen fortsetter etter administratoravklaring.",
            }
          : {}),
    ...
    if (closes || resumes) {
      const subject = closes
        ? contractRequest.kind === "withdrawal"
          ? "Bekreftelse på behandlet angremelding"
          : "Bekreftelse på avsluttet bestilling"
      ...
      const message = await payload.create({
        ...
        data: {
          lead: leadId,
          replyToMessage:
            relationId(contractRequest.sourceMessage) || undefined,
          direction: "outbound",
          category: "follow_up",
          ...
          idempotencyKey: makeIdempotencyKey("contract-request.decision", {
            requestId: contractRequest.id,
            decision: parsed.data.decision,
          }),
          aiAssisted: false,
          aiAnalysis: {
            customerContractRequestId: contractRequest.id,
            decision: parsed.data.decision,
          },
          ...(closes
            ? { approvedBy: user.id, approvedAt: now, queuedAt: now }
            : {}),
        },
      });
        ...
        if (provider.health().status === "ready") {
          try {
            ...
            captureException(error, {
              route: "POST /api/admin/customer-contract-requests/[id]",
              operation: "closure-confirmation-delivery",
              correlationId,
            });
          }
        }
    ...
    await updateCaseState(payload, {
      leadId,
      actorId: user.id,
      command: "resolve_customer_contract_request",
      idempotencyKey: `${correlationId}:contract-request:${contractRequest.id}`,
      patch: closes
        ? {
            status: "closed",
            ...
            nextAction:
              "Sluttbekreftelsen er sendt eller lagt i utsendingskø. Saken kan arkiveres.",
            nextActionAt: null,
            closedAt: now,
          }
        : resumes
          ? {
              ...
              nextAction: workOrder
                ? "Kontroller gjenopptatt arbeidsplan og send kundemeldingen."
                : "Fortsett planleggingen og send kundemeldingen.",
              nextActionAt: now,
              closedAt: null,
            }
          : parsed.data.decision === "alternative" && revisedPackage
            ...
            : {
                ...
                nextAction:
                  parsed.data.decision === "alternative"
                    ? "Lag et alternativt tilbud. Arbeidet forblir sperret."
                    : "Følg opp på valgt dato. Arbeidet forblir sperret.",
                nextActionAt: parsed.data.followUpAt || now,
                closedAt: null,
              },
    });
