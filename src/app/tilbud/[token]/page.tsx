...
export const metadata: Metadata = { title: "Personlig tilbud | Takfornyelse", robots: { index: false, follow: false } };
...
export default async function CustomerQuotePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!view) notFound();
  return <html className={manrope.variable} lang="no"><body className="min-h-svh bg-background font-sans text-foreground antialiased"><CustomerQuote
    ...
    terms={view.snapshot.terms} signedAt={view.signedAt}
  /></body></html>;
}
