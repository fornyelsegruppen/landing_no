...
- `STATUS`: `MANUAL_PREVIEW_CANARY_PASS`
...
- Stock-image selection and Pexels provenance were not accepted as evidence;
  the Pexels field was empty in the observed editor state.
...
`FEATURE_SEO_AUTO_PUBLISH=false`, verify idempotent duplicate handling, verify
image provenance, and record that the generated article remains unpublished.
This requires no Production activation but must not be represented as complete
until the actual scheduler result is observed.
